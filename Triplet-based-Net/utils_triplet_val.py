...
class Trainer(object):
    ...
    def _iteration(self, data_loader, ep ,is_train=True):
        ...
        for i,(img1,img2,target) in enumerate(data_loader):
            ...
            output1 = output1.permute(0,2,3,1).contiguous().view(-1,c)
            output2 = output2.permute(0,2,3,1).contiguous().view(-1,c)

            out = F.pairwise_distance(output1,output2)
            self.logger.info(str(i)+'/'+str(len(data_loader)))
            out = out.cpu().data.numpy()
            path = '/mnt/lustre/pangchao/cd_paper/code_v2.0/cexp4/tripletloss/'
            np.save(path + str(i) + '_dist.npy', out )
            np.save(path + str(i) + '_target.npy', target)
